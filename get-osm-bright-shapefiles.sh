...
mkdir -p /home/renderer/src/style-backup/shp
cd /home/renderer/src/style-backup/shp || exit
...
if [ ! -s ne_10m_populated_places.zip ] ; then
	...
	if $WET_RUN ; then
		curl_req -o ne_10m_populated_places.zip "https://naciscdn.org/naturalearth/10m/cultural/ne_10m_populated_places.zip"
	fi
fi
